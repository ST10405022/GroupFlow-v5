...
class AuthenticationServiceTest {
    ...
    @Test
    fun `register new user succeeds`() =
        runTest {
    ...
    @Test
    fun `register existing user fails`() =
        runTest {
    ...
    @Test
    fun `login with valid user succeeds`() =
        runTest {
            authService.register("test@example.com", "password", "John Doe", Role.PATIENT.name)
            authService.logout()
    ...
    @Test
    fun `login with invalid user fails`() =
        runTest {
    ...
    @Test
    fun `getCurrentUserProfile returns logged in user`() =
        runTest {
            authService.register("test@example.com", "password", "John Doe", Role.PATIENT.name)
    ...
    @Test
    fun `getCurrentUserProfile fails when no user logged in`() =
        runTest {
            val result = authService.getCurrentUserProfile()
            assertTrue(result.isFailure)
        }
