...
class NotificationServiceTest {
        ...
        notificationService =
            object : NotificationService {
                ...
                override suspend fun getNotificationsForUser(userId: String): Result<List<Notification>> =
                    Result.success(notifications[userId] ?: emptyList())
            }
    }

    @Test
    fun `pushNotificationForUser should store notification`() =
        runBlocking {
            val notification =
                Notification(
                    id = "1",
    ...
    @Test
    fun `getNotificationsForUser should return empty list when none exist`() =
        runBlocking {
